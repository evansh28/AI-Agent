...
async def TextToAudioFile(text) -> None: 
    ...
    if os.path.exists(file_path):
        try:
            ...
            print("File is in use, cannot delete.")
...
def TTS(Text, func=lambda r=None: True):
    try:
        asyncio.run(TextToAudioFile(Text))  # Ensuring async execution in the main thread
...
def TextToSpeech(Text, func=lambda r=None: True):
    ...
    Data = [sentence.strip() for sentence in Text.split(".") if sentence.strip()]
