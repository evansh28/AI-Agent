...
def QueryModifier(Query):
    print("Got Something here {Query}")
...
class ChatSection(QWidget):
        ...
        def __init__(self):
            ...
            self.old_chat_message = ""  # Initialize global variable
            
            layout = QVBoxLayout(self)
            layout.setContentsMargins(10, 40, 40, 100)  # Updated negative margins
            layout.setSpacing(10)  # Updated spacing
            ...
            self.chat_text_edit.setStyleSheet("color: blue;")  # Set text color directly
            layout.addWidget(self.chat_text_edit)

            self.gif_label = QLabel()
            self.gif_label.setStyleSheet("border: none;")
            movie = QMovie(GraphicsDirectoryPath('Jarvis.gif'))
            movie.setScaledSize(QSize(480, 270))  # Set max gif size
            self.gif_label.setAlignment(Qt.AlignRight | Qt.AlignBottom)
            self.gif_label.setMovie(movie)
            movie.start()
            layout.addWidget(self.gif_label)  # Added only once

            self.label = QLabel("")
            self.label.setStyleSheet("color: white; font-size:16px; margin-right: 195px; border: none; margin-top: -30px;")
            self.label.setAlignment(Qt.AlignRight)
            layout.addWidget(self.label)  # Added only once
            ...
            self.timer.start(1000)  # Increased interval to avoid performance issues
